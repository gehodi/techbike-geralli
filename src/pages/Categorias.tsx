  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Categorías de Inventario</h1>
          <p className="text-slate-500 mt-1">Gestión de categorías de productos</p>
        </div>
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          <h2 className="text-lg font-semibold text-slate-800 mb-4">{editingId ? 'Editar Categoría' : 'Nueva Categoría'}</h2>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-slate-500">{searchTerm ? 'No se encontraron categorías' : 'No hay categorías registradas'}</div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-200">
                {filtered.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600">{c.descripcion || '-'}</td>
