...
export default function Cotizaciones() {
  ...
  async function fetchCotizaciones() {
    try {
      ...
      // Búsqueda simple igual que Inventario
      let countQuery = supabase.from('cotizaciones').select('*', { count: 'exact', head: true })
      if (searchTerm) {
        const term = `%${searchTerm}%`
        countQuery = countQuery.or(`notas.ilike.${term},estado.ilike.${term},id::text.ilike.${term}`)
      }
      ...
      if (searchTerm) {
        const term = `%${searchTerm}%`
        query = query.or(`notas.ilike.${term},estado.ilike.${term},id::text.ilike.${term}`)
      }
